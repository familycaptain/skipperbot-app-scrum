...
Ported from the platform's ``data_layer/scrum.py``. Functionally identical;
the only difference is that every query is routed through the ``*_in_schema``
helpers from ``app_platform.db`` so the scrum app's rows land in (and read
from) the ``app_scrum`` schema. ``data_layer.links`` stays as-is — links are
platform infrastructure living in ``public.*``.
"""
...
def respond_to_item(item_id: str, response: str) -> dict | None:
    ...
    row = fetch_one_in_schema(SCHEMA, "SELECT * FROM scrum_items WHERE id = %s", (item_id,))
